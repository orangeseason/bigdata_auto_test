...
case class hivInfo(sql:String,drop:Array[String],rename:Map[String,String])
case class cassaInfo(keyspace:String,table:String,condition:Array[String])
case class CountInfo(name:String)
case class RecordInfo(name:String, num:Int, primaryKey:Array[String], field:Array[String], ignor:Array[String] )
...
object xmlUtil {
  ...
  def parseTCElement(TC:scala.xml.Node):TcInfo={
    ...
    val hive_rename_list = (hive_Info\"rename")(0).text.replace(" ","").split(",")
    ...
    val hive = new hivInfo(hive_sql,hive_drop, rename_map)
    ...
    val cassa = new cassaInfo(cassan_ks,cassan_table,cassan_condition)
    ...
    val record_num = (compare_record\"num")(0).text.replace(" ","").toInt
    ...
    val record = new RecordInfo(record_name,record_num,record_primaryKey,record_field,record_ignor)
